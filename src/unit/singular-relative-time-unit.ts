...
const VALID_SINGULAR_RELATIVE_TIME_UNIT_VALUES: SingularRelativeTimeUnit[] = [
  "second",
  "minute",
  "hour",
  "day",
...
export function singularRelativeTimeUnit(
  ...
  if (
    !VALID_SINGULAR_RELATIVE_TIME_UNIT_VALUES.some(
      validUnit => validUnit === unit
    )
  ) {
    throw new RangeError(
      `Unit: '${unit}' must be one of: ${VALID_SINGULAR_RELATIVE_TIME_UNIT_VALUES.map(
        val => `"${val}"`
      ).join(", ")}`
    );
  }
