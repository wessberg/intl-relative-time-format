...
import {
  getInternalSlot,
  hasInternalSlot,
  RELATIVE_TIME_FORMAT_STATIC_INTERNALS,
  setInternalSlot
...
import {
  getDefaultLocale,
  setDefaultLocale
} from "../default-locale/get-default-locale";
...
export class RelativeTimeFormat {
  ...
  constructor(
    ...
    options?: Partial<RelativeTimeFormatOptions>
  ) {
    ...
    const matcher =
      options.localeMatcher != null ? options.localeMatcher : "best fit";
    ...
    if (!(fields instanceof Object)) {
      throw new TypeError(
        `Expected the LocaleDataEntry for locale: '${dataLocale}' to be an Object`
      );
    }
   ...
   * @param {Locale} locale
   */
  protected static __addLocaleData({
    data,
    locale
  }: InputLocaleDataEntry): void {
    ...
    if (
      !RELATIVE_TIME_FORMAT_STATIC_INTERNALS.availableLocales.includes(locale)
    ) {
  ...
  public format(value: number, unit: RelativeTimeUnit): string {
    ...
    if (!(relativeTimeFormat instanceof Object)) {
      throw new TypeError(
        `Method Intl.RelativeTimeFormat.prototype.format called on incompatible receiver ${this.toString()}`
      );
    }
  ...
  public formatToParts(value: number, unit: RelativeTimeUnit): UnitPartitions {
    ...
    if (!(relativeTimeFormat instanceof Object)) {
      throw new TypeError(
        `Method Intl.RelativeTimeFormat.prototype.formatToParts called on incompatible receiver ${this.toString()}`
      );
    }
  ...
  public resolvedOptions(): ResolvedRelativeTimeFormatOptions {
    ...
    if (!(relativeTimeFormat instanceof Object)) {
      throw new TypeError(
        `Method Intl.RelativeTimeFormat.prototype.resolvedOptions called on incompatible receiver ${this.toString()}`
      );
    }
