...
export function resolveLocale(
  ...
  for (const key of relevantExtensionKeys) {
    ...
    if (!isRecord(foundLocaleData)) {
      throw new TypeError(
        `LocaleData for locale: '${foundLocale}' must be an object`
      );
    }
    ...
    if (!isList(keyLocaleData)) {
      throw new TypeError(
        `key: '${key}' in LocaleData for locale: '${foundLocale}' must be indexable`
      );
    }
    ...
    if (typeof value !== "string" && value !== null) {
      throw new TypeError(
        `value: '${value}' for key: '${key}' in LocaleData for locale: '${foundLocale}' must be a string or null`
      );
    }
    ...
    if ("key" in options) {
      ...
      if (typeof optionsValue !== "string" && optionsValue != null) {
        throw new TypeError(
          `options value: '${optionsValue}' must be a string, undefined, or null`
        );
      }
