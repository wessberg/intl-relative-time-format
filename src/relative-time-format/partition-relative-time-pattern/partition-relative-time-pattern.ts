...
} from "../../unit/singular-relative-time-unit";
...
export function partitionRelativeTimePattern(
  ...
  if (!hasInternalSlot(relativeTimeFormat, "initializedRelativeTimeFormat")) {
    throw new TypeError(
      `Internal function called on incompatible receiver ${relativeTimeFormat.toString()}`
    );
  }
  ...
  if (patterns == null) {
    throw new TypeError(
      `Could not match entry: '${entry}' inside fields for locale: '${getInternalSlot(
        relativeTimeFormat,
        "locale"
      )}'`
    );
